...
open class DKKeyFrameAnimation: CAKeyframeAnimation {
    ...
    convenience init(keyPath path: String!) {
        self.init()
        keyPath = path
        functionBlock = DKKeyframeAnimationFunctionLinear
    }
    ...
    internal func createValueArray() {
        if let fromValue: AnyObject = self.fromValue, let toValue: AnyObject = self.toValue {
            if valueIsKindOf(NSNumber.self) {
                values = valueArrayFor(startValue: CGFloat(fromValue.floatValue), endValue: CGFloat(toValue.floatValue)) as [AnyObject]
                ...
                let redValues = valueArrayFor(startValue: red.start, endValue: red.end) as! [CGFloat]
                let greenValues = valueArrayFor(startValue: green.start, endValue: green.end) as! [CGFloat]
                let blueValues = valueArrayFor(startValue: blue.start, endValue: blue.end) as! [CGFloat]
                let alphaValues = valueArrayFor(startValue: alphaStart!, endValue: alphaEnd!) as! [CGFloat]

                values = colorArrayFrom(redValues: redValues, greenValues: greenValues, blueValues: blueValues, alphaValues: alphaValues) as [AnyObject]
                ...
                if valueType.contains("CGRect") {
                    ...
                    let xValues = valueArrayFor(startValue: (fromRect?.origin.x)!, endValue: (toRect?.origin.x)!) as! [CGFloat]
                    let yValues = valueArrayFor(startValue: (fromRect?.origin.y)!, endValue: (toRect?.origin.x)!) as! [CGFloat]
                    let widthValues = valueArrayFor(startValue: (fromRect?.size.width)!, endValue: (toRect?.size.width)!) as! [CGFloat]
                    let heightValues = valueArrayFor(startValue: (fromRect?.size.height)!, endValue: (toRect?.size.height)!) as! [CGFloat]

                    values = rectArrayFrom(xValues: xValues, yValues: yValues, widthValues: widthValues, heightValues: heightValues) as [AnyObject]

                } else if valueType.contains("CGPoint") {
                    let fromPoint = self.fromValue.cgPointValue
                    let toPoint = self.toValue.cgPointValue
                    let path = createPathFromXYValues(valueArrayFor(startValue: (fromPoint?.x)!, endValue: (toPoint?.x)!), yValues: valueArrayFor(startValue: (fromPoint?.y)!, endValue: (toPoint?.y)!))
                    self.path = path
                } else if valueType.contains("CGSize") {
                    let fromSize = self.fromValue.cgSizeValue
                    let toSize = self.toValue.cgSizeValue
                    let path = createPathFromXYValues(valueArrayFor(startValue: (fromSize?.width)!, endValue: (toSize?.width)!), yValues: valueArrayFor(startValue: (fromSize?.height)!, endValue: (toSize?.height)!))
                    self.path = path
                    ...
                    let m11 = valueArrayFor(startValue: (fromTransform?.m11)!, endValue: (toTransform?.m11)!)
                    let m12 = valueArrayFor(startValue: (fromTransform?.m12)!, endValue: (toTransform?.m12)!)
                    let m13 = valueArrayFor(startValue: (fromTransform?.m13)!, endValue: (toTransform?.m13)!)
                    let m14 = valueArrayFor(startValue: (fromTransform?.m14)!, endValue: (toTransform?.m14)!)
                    let m21 = valueArrayFor(startValue: (fromTransform?.m21)!, endValue: (toTransform?.m21)!)
                    let m22 = valueArrayFor(startValue: (fromTransform?.m22)!, endValue: (toTransform?.m22)!)
                    let m23 = valueArrayFor(startValue: (fromTransform?.m23)!, endValue: (toTransform?.m23)!)
                    let m24 = valueArrayFor(startValue: (fromTransform?.m24)!, endValue: (toTransform?.m24)!)
                    let m31 = valueArrayFor(startValue: (fromTransform?.m31)!, endValue: (toTransform?.m31)!)
                    let m32 = valueArrayFor(startValue: (fromTransform?.m32)!, endValue: (toTransform?.m32)!)
                    let m33 = valueArrayFor(startValue: (fromTransform?.m33)!, endValue: (toTransform?.m33)!)
                    let m34 = valueArrayFor(startValue: (fromTransform?.m34)!, endValue: (toTransform?.m34)!)
                    let m41 = valueArrayFor(startValue: (fromTransform?.m41)!, endValue: (toTransform?.m41)!)
                    let m42 = valueArrayFor(startValue: (fromTransform?.m42)!, endValue: (toTransform?.m42)!)
                    let m43 = valueArrayFor(startValue: (fromTransform?.m43)!, endValue: (toTransform?.m43)!)
                    let m44 = valueArrayFor(startValue: (fromTransform?.m44)!, endValue: (toTransform?.m44)!)

                    values = createTransformArrayFrom(
    ...
    fileprivate func createTransformArrayFrom(
        ...
        m41: NSArray, m42: NSArray, m43: NSArray, m44: NSArray
    ) -> NSArray {
    ...
    fileprivate func valueIsKindOf(_ klass: AnyClass) -> Bool {
        return fromValue.isKind(of: klass) && toValue.isKind(of: klass)
    }
    ...
    fileprivate func valueArrayFor(startValue: CGFloat, endValue: CGFloat) -> NSArray {
        ...
        let steps = Int(ceil(Double(kFPS) * duration)) + 2
